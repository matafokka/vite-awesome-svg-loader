...
Put all images into a single `<svg>` node (sprite sheet) and wrap each image with a `<symbol>` element.
This improves performance by reducing DOM tree size and enabling instancing.
